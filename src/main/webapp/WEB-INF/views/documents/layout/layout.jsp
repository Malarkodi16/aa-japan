	...
	<script type="text/javascript">
		$(function() {
			...
			$('.recycle-nav-box').on(
					...
					function() {
						localStorage.setItem(
								"aaj-document-recycle-dashboard-active-nav", $(
										this).data('nav'));
					})
		})
		function setState() {
			if (window.location.pathname == myContextPath
					+ '/documents/tracking/not-received') {
				localStorage.setItem("aaj-document-dashboard-active-nav", '0');
			}
			if (window.location.pathname == myContextPath
					+ '/documents/recycle/claim') {
				localStorage.setItem(
						"aaj-document-recycle-dashboard-active-nav", '0');
			}
			...
			var navBox = $('#document-status-wrapper').find(
					'div.nav-box[data-nav="' + dashboardNav + '"]');
			navBox.addClass('border-solid').removeClass('blur');

			var dashboardRecycleClaim = localStorage
					.getItem("aaj-document-recycle-dashboard-active-nav");
			var navBox = $('#document-status-wrapper').find(
					'div.recycle-nav-box[data-nav="' + dashboardRecycleClaim
							+ '"]');
			navBox.addClass('border-solid').removeClass('blur');
